...
void CPlayerAttack::InitPlayerAttackMove(void)
{
	...
	//プレイヤーが死んだとき
	bool Attack = CBullet::GetPlayerAttack();
	bool Attack2 = CPlayer::GetPlayerAttack();
	bool Attacck3 = CLaser::GetPlayerAttack();
	...
	if (m_playerattack == PLAYERATTACK_RIGHT)
	{
		...
		if (pInputGamePad->IsButtonPush(CInput::KEYINFO_OK) == true || pInputKeyboard->GetPress(DIK_Z) == true)
		{
			...
			if (m_shotcount % 7 == 0)
			{
				...
				//pSound->Play(pSound->SOUND_SHOT_SE000);
			}
		}
	...
	if (Attack == true || Attack2 == true || Attacck3 == true)
	{
		//消去
		Uninit();
	}
