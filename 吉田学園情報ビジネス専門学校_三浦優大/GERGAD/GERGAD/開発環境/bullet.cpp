	...
	for (int nCntBullet = 0; nCntBullet <702; nCntBullet++, pBullet++)
	{
		if (pBullet->bUse == false)//弾を使用しているかどうか
		{
			if (nType == BULLETTYPE_ENEMY_HOMING)
			{
				...
				pVtx[0].col = g_colorBullet = D3DXCOLOR(0.0f + col, 0.0f + col, 0.0f + col, 0.0f + col);
				pVtx[1].col = g_colorBullet = D3DXCOLOR(0.0f + col, 0.0f + col, 0.0f + col, 0.0f + col);
				pVtx[2].col = g_colorBullet = D3DXCOLOR(0.0f + col, 0.0f + col, 0.0f + col, 0.0f + col);
				pVtx[3].col = g_colorBullet = D3DXCOLOR(0.0f + col, 0.0f + col, 0.0f + col, 0.0f + col);
