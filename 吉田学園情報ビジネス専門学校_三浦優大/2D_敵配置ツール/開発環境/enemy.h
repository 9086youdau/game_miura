...
public:
	...
	~CEnemy();                                //デストラクタ
	static CEnemy *Create(D3DXVECTOR3 pos, float fHight, float fWidth, ENEMYTYPE enemyType,float fMove, int nHoming);//敵の生成処理

	HRESULT Init(D3DXVECTOR3 pos, float fHight, float fWidth,ENEMYTYPE enemyType,float fMove, int nHoming);       //初期化処理
