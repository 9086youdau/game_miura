...
CModel *CPlayer3D::m_apModel[MAX_PLAYER_PARTS] = {};
char *CPlayer3D::m_SelectData[MAX_PLAYER_PARTS + 1][256] = {};
int CPlayer3D::m_DataCount = 0;
float CPlayer3D::m_aOffset[MAX_PLAYER_PARTS][6] = {};
...
CPlayer3D::CPlayer3D(int nPriority, int nType)
{
	...
	m_bData = false;
	m_SelectData[0][256] = "●";
	for (int nCntData = 1; nCntData < MAX_PLAYER_PARTS; nCntData++)
	{
		//選択しているモデル表示ログ設定
...
void CPlayer3D::MotionTool(void)
{
	...
	else
	{
		...
		if (m_DataCount >= MAX_PLAYER_PARTS)
		{
			//カウントを0に戻す
			m_DataCount = 0;
		}
	...
	else
	{
		...
		{
			//カウントを0に戻す
			m_DataCount = MAX_PLAYER_PARTS - 1;
		}
	...
	if (pInputKeyboard->GetTrigger(DIK_DOWN) == true)
	{
		...
		{
			// マテリアル情報に対するポインタを取得
			pMat = (D3DXMATERIAL*)m_apModel[MAX_PLAYER_PARTS - 1]->GetBuff()->GetBufferPointer();

			//マテリアルの色変更
			pMat->MatD3D.Diffuse = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);

			// 頂点バッファをアンロック
			m_apModel[MAX_PLAYER_PARTS - 1]->GetMash()->UnlockVertexBuffer();
		}
		...
		if (m_nCntMat >= MAX_PLAYER_PARTS - 1)
		{
			//カウントを-1に戻してあげる
			m_nCntMat = -1;
		}
	...
	if (pInputKeyboard->GetTrigger(DIK_UP) == true)
	{
		...
		if (m_nCntMat <= 0)
		{
			m_nCntMat = MAX_PLAYER_PARTS;
		}

		//値を保存
		m_nCntMatOld = m_nCntMat;

		if (m_nCntMat >= MAX_PLAYER_PARTS)
		{
			m_nCntMatOld = 0;
		}
...
void CPlayer3D::PlayerMove(void)
{
	...
	if (pInputKeyboard->GetPress(DIK_W) == true)
	{
		...
		{
			if (m_MotionChange == MOTIONTYPE_MOVE)
			{
				m_GetKey = 0;
				m_Key = 0;
				m_nCounterMotion = 0;
	...
	else if (pInputKeyboard->GetPress(DIK_S) == true)
	{
		...
		if (pInputKeyboard->GetPress(DIK_A) == true)
		{// 左下方向
			if (m_MotionType == MOTIONTYPE_MOVE)
			{
				m_GetKey = 0;
				m_Key = 0;
				m_nCounterMotion = 0;
