...
void CRenderer::Update(void)
{
	...
	if (pInputKeyboard->GetTrigger(DIK_UP) == true)
	{
		...
		if (m_nCnt2 >= 7 && m_SideCnt <= 7 + m_nMaxPlayerPart - 1)
		{
			//カウントダウン
			m_nMaxPlayerPart--;
			//カウントアップ
...
//#ifdef _DEBUG
		...
		if (m_nCnt < 0)
		{
			for (int nCntModelParent = MAX_PLAYER_PARTS + m_nCnt2; nCntModelParent < MAX_PLAYER_PARTS + m_nMaxPlayerPart; nCntModelParent++)
			{
