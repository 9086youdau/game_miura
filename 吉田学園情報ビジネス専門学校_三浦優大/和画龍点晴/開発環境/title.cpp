...
CTitle::CTitle()
{
	...
	m_nCounterAnim = 0.0f;
	m_pos = D3DXVECTOR3(950.0f, 0.0f, 1.0f);
	m_posui = D3DXVECTOR3(-120.0f, 950.0f, 1.0f);
	m_moveui = D3DXVECTOR3(0.0f, 0.0f, 1.0f);
}
//=============================================================================
//デストラクタ
//=============================================================================
...
void CTitle::SetVertex(void)
{
	for (int nCntTitle = 0; nCntTitle < TITLETYPE_MAX; nCntTitle++)
	{
		if (nCntTitle == TITLETYPE_BG)
		{
			...
			m_pScene2D[TITLETYPE_BG]->Init(D3DXVECTOR3(0.0f, 0.0f, 1.0f), SCREEN_HEIGHT, SCREEN_WIDTH);
		...
		else if (nCntTitle == TITLETYPE_MOVEBG)
		{
			...
			m_pScene2D[TITLETYPE_MOVEBG]->Init(D3DXVECTOR3(0.0f, 0.0f, 1.0f), SCREEN_HEIGHT, SCREEN_WIDTH);
		...
		else if (nCntTitle == TITLETYPE_MOVEBG2)
		{
			...
			m_pScene2D[TITLETYPE_MOVEBG2]->Init(D3DXVECTOR3(0.0f, 0.0f, 1.0f), SCREEN_HEIGHT, SCREEN_WIDTH);
		...
		else if (nCntTitle == TITLETYPE_GAME)
		{
			...
			m_pScene2D[TITLETYPE_GAME]->Init(D3DXVECTOR3(1300.0f, 750.0f, 1.0f), 120.0f, 600.0f);
		...
		else if (nCntTitle == TITLETYPE_EXIT)
		{
			...
			m_pScene2D[TITLETYPE_EXIT]->Init(D3DXVECTOR3(1300.0f, 900.0f, 1.0f), 120.0f, 600.0f);
...
void CTitle::TitleSelect(void)
{
	...
	if (m_titleType == TITLETYPE_GAME)
	{
		...
		if (m_nCount % 30 == 0)//間隔
		{
			m_pScene2D[TITLETYPE_GAME]->SetCol(D3DXCOLOR(255, 0, 0, 155));
		}

		else if (m_nCount % 15 == 0 && m_nCount % 30)//間隔
		{
			m_pScene2D[TITLETYPE_GAME]->SetCol(D3DXCOLOR(255, 0, 0, 255));
		}
	...
	else if (m_titleType == TITLETYPE_EXIT)
	{
		...
		if (m_nCount % 30 == 0)//間隔
		{
			m_pScene2D[TITLETYPE_EXIT]->SetCol(D3DXCOLOR(255, 0, 0, 155));
		}

		else if (m_nCount % 15 == 0 && m_nCount % 30)//間隔
		{
			m_pScene2D[TITLETYPE_EXIT]->SetCol(D3DXCOLOR(255, 0, 0, 255));
		}
	...
	if (pInputGamePad->IsButtonDown(CInput::KEYINFO_UP) == true || pInputKeyboard->GetTrigger(DIK_UP) == true)
	{
		if (m_titleType == TITLETYPE_GAME)
		{
			m_pScene2D[TITLETYPE_GAME]->SetCol(D3DXCOLOR(255, 255, 255, 255));
			m_titleType = TITLETYPE_EXIT;
		}
		else if (m_titleType == TITLETYPE_EXIT)
		{
			m_pScene2D[TITLETYPE_EXIT]->SetCol(D3DXCOLOR(255, 255, 255, 255));
			m_titleType = TITLETYPE_GAME;
		}
	}
