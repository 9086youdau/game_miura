...
HRESULT InitTitle(void)
{
	...
	//テキスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/back-buildings.png", &g_pTextureTitle[0]);
	...
	for (int nCount = 0; nCount < 3; nCount++)
	{
		...
		g_colorTitle = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
		g_colorBG = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
		g_colorTitlelogo = D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f);
...
void UpdateTitle(void)
{
	...
	{
		SetFade(FADE_OUT, MODE_TUTOLIAL);
	}
	...
	//頂点座標の設定
	pVtx[4].pos = D3DXVECTOR3(g_PosTitle.Pos.x - (700 / 2), g_PosTitle.Pos.y + (300 / 2), 0.0f);//2Dは0.0固定
	pVtx[5].pos = D3DXVECTOR3(g_PosTitle.Pos.x - (700 / 2), g_PosTitle.Pos.y - (300 / 2), 0.0f);
	pVtx[6].pos = D3DXVECTOR3(g_PosTitle.Pos.x + (1000 / 2), g_PosTitle.Pos.y + (300 / 2), 0.0f);
	pVtx[7].pos = D3DXVECTOR3(g_PosTitle.Pos.x + (1000 / 2), g_PosTitle.Pos.y - (300 / 2), 0.0f);//2Dは0.0固定
