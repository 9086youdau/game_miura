...
HRESULT InitPause(void)
{
	...
	//テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, "data\\TEXTURE\\unnamed.png", &g_apTexPause[0]);
	...
	D3DXCreateTextureFromFile(pDevice, "data\\TEXTURE\\111.png", &g_apTexPause[PAUSE_MENE_POINT]);
...
void UpdatePause(void)
{
	...
	if (GetKeyboardPress(DIK_RETURN) == true)
	{
		...
		else if (g_nPointNow == PAUSE_MENE_RETRY)
		{
			//StopSound(SOUND_LABEL_BGM000);	//サウンドを止める 
			SetFade(FADE_OUT, MODE_TITLE);	//ゲーム画面に飛ぶ＆フェードアウト
		}

		else if (g_nPointNow == PAUSE_MENE_QUIT)
		{
			//StopSound(SOUND_LABEL_BGM001);	//サウンドを止める
			SetFade(FADE_OUT, MODE_GAME);//フェードアウトをしてリザルト画面に飛ぶ
		}
