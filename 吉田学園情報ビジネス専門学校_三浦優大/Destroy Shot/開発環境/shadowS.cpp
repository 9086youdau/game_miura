...
HRESULT CShadowS::Init(D3DXVECTOR3 pos, float fSizeX, float fSizeY)
{
	...
	//model読み込み
	D3DXLoadMeshFromX("data/MODEL/shadow.x", D3DXMESH_SYSTEMMEM, pDevice, NULL, &m_pBuffMat, NULL, &m_nNumMat, &m_pMesh);
	...
	//頂点カラーの設定
	pVtx[0].col = D3DXCOLOR(0, 0, 0, 255);
	pVtx[1].col = D3DXCOLOR(0, 0, 0, 255);
	pVtx[2].col = D3DXCOLOR(0, 0, 0, 255);
	pVtx[3].col = D3DXCOLOR(0, 0, 0, 255);
...
{
	//位置
	m_pos = D3DXVECTOR3(CPlayer3D::GetPosition().x, CPlayer3D::GetPosition().y + 10.0f, CPlayer3D::GetPosition().z);
}
