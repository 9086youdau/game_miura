...
HRESULT CRanking::Init(D3DXVECTOR3 pos, float fSizeX, float fSizeY)
{
	for (int nCount = 0; nCount < 5; nCount++)
	{
		...
		{
			//番号を生成
			m_apRankingNumber[nCount][nCnt] = CNumber::Create(D3DXVECTOR3(pos.x - (nCnt * 50.0f), pos.y + (nCount * 120.0f), 0.0f), 30, 20);
		}
	}
	...
	{
		//番号を生成
		m_apMaxScoreNumber[nCnt] = CNumber::Create(D3DXVECTOR3(1800.0f - (nCnt * 50.0f), 355.0f, 0.0f), 30, 20);
	}
	...
	//残基と照らし合わせてスコアを表示
	m_nMaxRankScore = CScore::GetScore() * ((CGame::GetLife() + 1));
...
void CRanking::Uninit(void)
{
	...
	{
		// 頂点バッファの開放
		if (m_apMaxScoreNumber[nCnt] != NULL)
		{
			m_apMaxScoreNumber[nCnt]->Uninit();
			delete m_apMaxScoreNumber[nCnt];
			m_apMaxScoreNumber[nCnt] = NULL;
		}
	}
